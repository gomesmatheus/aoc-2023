...
func main() {
    ...
    re := regexp.MustCompile("([0-9]+\\s(red|green|blue))")
    /*
    redCubesLimit := 12
    greenCubesLimit := 13
    blueCubesLimit := 14
    */
    ...
    for i, game := range games {
        ...
        for _, reveal := range reveals {
            cubes := re.FindAllString(reveal, -1)
            //fmt.Println("Game ", gameNumber, " - ", cubes)

            for _, cube := range cubes {
                segments := strings.Split(cube, " ")
                fmt.Println(segments)
                color := segments[1]
                amount, _ := strconv.Atoi(segments[0])

                if color == "red" && amount > fewerRedCubes {
                    fewerRedCubes = amount
                }
                if color == "green" && amount > fewerGreenCubes {
                    fewerGreenCubes = amount
                }
                if color == "blue" && amount > fewerBlueCubes {
                    fewerBlueCubes = amount
                }
                
                /*
                if color == "red" && amount > redCubesLimit {
                    possibleGame = false
                    break
                }
                if color == "green" && amount > greenCubesLimit {
                    possibleGame = false
                    break
                }
                if color == "blue" && amount > blueCubesLimit {
                    possibleGame = false
                    break
                }
                fmt.Println(gameNumber, color, amount, possibleGame)
                */
            }
        ...
        fmt.Println(gameNumber, fewerRedCubes, fewerBlueCubes, fewerGreenCubes)
        fmt.Println("somando", fewerGreenCubes * fewerRedCubes * fewerBlueCubes)

        sum += (fewerBlueCubes * fewerRedCubes * fewerGreenCubes)
        /* 
        if possibleGame {
            fmt.Println("Adding game ", gameNumber)
            sum += gameNumber
        }
        */
