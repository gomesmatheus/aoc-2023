...
// first and last number of each line summed
...
func main() {
    ...
    re := regexp.MustCompile("[0-9]|(twone)|(oneight)|(two)|(eightwo)|(one)|(three)|(four)|(five)|(six)|(seven)|(nine)|(eight)")
    sum := 0
    for _, line := range lines {
        founds := re.FindAllString(line, -1)
        //fmt.Println(founds, len(founds), founds[0], founds[len(founds) - 1])
        
        first := founds[0]
        last := founds[len(founds) - 1]
        if first == "twone" {
            first = "two"
        } else if first == "oneight" {
            first = "one"
    
        } else if first == "eightwo" {
            first = "eight"
        }

        if last == "twone" {
            last = "one"
        } else if last == "oneight" {
            last = "eight"
        } else if last == "eightwo" {
            last = "two"
        }

        fmt.Println(founds, first, last, line)
        //fmt.Println(founds, len(founds), parseToDigit(founds[0]), parseToDigit(founds[len(founds) - 1]))
        num, _ := strconv.Atoi(parseToDigit(first) + parseToDigit(last))
        fmt.Println(num)
        sum += num
    }
    fmt.Println(sum)
...
func parseToDigit(str string) string {
    if str == "one" {
        return "1"
    } else if str == "two" {
        return "2"
